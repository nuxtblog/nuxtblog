...
func (s *sPlugin) Install(ctx context.Context, repoUrl string) (*v1.PluginItem, error) {
	if err := requireAdmin(ctx); err != nil {
		return nil, err
	}
	...
	if err = json.Unmarshal(releaseData, &release); err != nil || release.ZipballURL == "" {
		return nil, gerror.NewCode(gcode.CodeInternalError,
			fmt.Sprintf("no published release found for %s/%s — create a GitHub Release first", owner, repo))
	}
...
type parseArchiveResult struct {
	...
	AllFiles map[string][]byte // all files in archive (for JS plugins that need full extraction)
...
type pluginYAMLManifest struct {
	...
	Type    string `yaml:"type"`     // js, yaml, ui, full
...
func (s *sPlugin) Uninstall(ctx context.Context, id string) error {
	if err := requireAdmin(ctx); err != nil {
		return err
	}

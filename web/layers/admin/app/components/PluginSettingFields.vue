  ...
  <template v-for="(group, groupIdx) in groups" :key="group.name ?? '__default__'">
    ...
    <template v-for="field in group.fields" :key="field.key">
      <UFormField v-if="isFieldVisible(field)" :label="field.label || field.key" :description="field.description" :required="field.required">
        ...
        <UTextarea v-else-if="field.type === 'textarea'" :model-value="String(values[field.key] ?? '')" :placeholder="field.placeholder" class="w-full" :rows="3" @update:model-value="updateField(field.key, $event)" />
        <UInput v-else-if="field.type === 'password'" type="password" :model-value="String(values[field.key] ?? '')" :placeholder="field.placeholder" class="w-full" @update:model-value="updateField(field.key, $event)" />
        <UInput v-else-if="field.type === 'number'" type="number" :model-value="String(values[field.key] ?? '')" :placeholder="field.placeholder" class="w-full" @update:model-value="updateField(field.key, Number($event))" />
        <UInput v-else :model-value="String(values[field.key] ?? '')" :placeholder="field.placeholder" class="w-full" @update:model-value="updateField(field.key, $event)" />
      </UFormField>
    </template>
  </template>
</template>

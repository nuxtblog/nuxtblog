<template>
  <AdminPageContainer>
    <AdminPageHeader :title="$t('admin.posts.categories.title')" :subtitle="$t('admin.posts.categories.subtitle')">
      <template #actions>
          ...
          <UCard>
            <template #header>
              <h2 class="text-base font-semibold text-highlighted">
                {{ isEditing ? $t('admin.posts.categories.edit_category') : $t('admin.posts.categories.add_category') }}
              </h2>
            </template>

            <form @submit.prevent="handleSubmit" class="space-y-4">
              <UFormField :label="$t('admin.posts.categories.name_label')" required>
                <UInput
              ...
              <div class="flex gap-3 pt-2">
                <UButton
                  ...
                  class="flex-1"
                >
                  {{ isEditing ? $t('admin.posts.categories.update_btn') : $t('admin.posts.categories.add_btn') }}
                </UButton>
                <UButton
                  v-if="isEditing"
                  type="button"
        ...
        <div class="lg:col-span-2">
          <UCard :ui="{ body: 'p-0' }">
            ...
            <div class="p-4 border-b border-default flex items-center justify-between gap-4">
              ...
              <ViewModeSwitcher
                v-model="viewMode"
                :modes="[
                  { value: 'tree', title: $t('admin.posts.categories.tree_view') },
                  { value: 'list', title: $t('admin.posts.categories.list_view') },
                ]"
              />
            </div>
            ...
            <div v-if="loading" class="p-4 space-y-3">
              <div v-for="i in 5" :key="i" class="flex items-center gap-4 p-4 bg-default border border-default rounded-md">
            ...
            <!-- 树状视图 -->
            <div v-else-if="viewMode === 'tree' && filteredTree.length > 0" class="p-4">
              <ClientOnly>
                <CategoryNode
                  v-model="visibleTree"
                  :parent-id="0"
            ...
            <!-- 列表视图 -->
            <div v-else-if="viewMode === 'list' && filteredList.length > 0" class="space-y-3 p-4">
              <div
              ...
              >
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3">
                      ...
                      <UBadge :label="$t('admin.posts.categories.post_count', { n: category.count })" size="sm" color="primary" variant="soft" />
                    </div>
                    <p v-if="category.description" class="text-sm text-muted mt-2 line-clamp-1">
                      {{ category.description }}
                    </p>
                    <div v-if="getParentName(category)" class="flex items-center gap-1.5 mt-2 text-xs text-muted">
                      <UIcon name="i-tabler-arrow-right" class="size-3.5" />
                      <span>{{ $t('admin.posts.categories.parent_label') }} <strong>{{ getParentName(category) }}</strong></span>
                    </div>
                  </div>
            ...
            <div v-else class="flex flex-col items-center justify-center py-16">
              <UIcon name="i-tabler-folder-open" class="size-16 text-muted mb-4" />
              <h3 class="text-lg font-medium text-highlighted mb-1">{{ $t('admin.posts.categories.no_categories') }}</h3>
            </div>
          </UCard>
          ...
          <UCard v-if="!loading && categories.length > 0" class="mt-4">
            <div class="grid grid-cols-3 gap-4 text-center">
              <div>
                <div class="text-2xl font-semibold text-highlighted">{{ categories.length }}</div>
                <div class="text-sm text-muted">{{ $t('admin.posts.categories.total_categories') }}</div>
              </div>
              <div>
                <div class="text-2xl font-semibold text-highlighted">
                  {{ categories.filter((t) => !t.parent_id).length }}
                </div>
                <div class="text-sm text-muted">{{ $t('admin.posts.categories.top_level') }}</div>
              </div>
              <div>
                <div class="text-2xl font-semibold text-highlighted">
                  {{ categories.reduce((sum, t) => sum + t.count, 0) }}
                </div>
                <div class="text-sm text-muted">{{ $t('admin.posts.categories.related_posts') }}</div>
              </div>
            </div>
          </UCard>
        </div>
        ...
        <template #content>
          <div class="p-6">
            <div class="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 class="font-semibold text-highlighted">{{ $t('admin.posts.categories.confirm_delete') }}</h3>
                <p class="text-sm text-muted mt-0.5">
                  {{ $t('admin.posts.categories.delete_confirm', { name: deleteConfirmTerm?.name }) }}
                </p>
              </div>
            </div>
...
import type { TermDetailWithChildren, TermWithLevel } from "~/types/models/term";
...
const filteredTree = computed(() => {
  ...
  const filterTree = (list: TreeNode[]): TreeNode[] =>
    list
      ...
      .filter((n) => ids.includes(n.term_id) || (n.children && n.children.length > 0));
...
const openEditModal = (term: TermDetailResponse) => {
  ...
  formData.value = { name: term.name, slug: term.slug, description: term.description };
};
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!slug) {
      toast.add({ title: t("admin.posts.categories.invalid_slug"), description: t("admin.posts.categories.invalid_slug_desc"), color: "error" });
      return;
    }
    if (isEditing.value && editingTerm.value) {
      ...
      toast.add({ title: t("admin.posts.categories.updated"), color: "success" });
    } else {
      ...
      toast.add({ title: t("admin.posts.categories.created"), color: "success" });
    }
    cancelEdit();
  } catch (err: any) {
    toast.add({ title: t("admin.posts.categories.submit_failed"), description: err?.message || t("common.retry"), color: "error" });
  } finally {
...
const confirmDelete = async () => {
  ...
  try {
    ...
    toast.add({ title: t("admin.posts.categories.delete_failed"), description: err?.message || t("common.retry"), color: "error" });
  } finally {

...
export function useEditorImageUpload(
  ...
  const category = options.imageCategory ?? "post";
  ...
  const uploadPendingImages = async (): Promise<void> => {
    ...
    toast.add({
      title: t("admin.editor.image_uploading"),
      color: "neutral",
      duration: 0,
      id: "img-upload",
    });
      ...
      const results = await Promise.all(
        matches.map(async ({ alt, src }) => {
          ...
          if (options.imageUploader) {
            cdnUrl = await options.imageUploader(dataUrlToFile(src, name));
          } else {
